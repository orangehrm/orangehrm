...
$new_version= new Versions();
//$new_db_version = new dbVersions();
//$new_file_version = new fileVersions();
$lastRecord = $new_version->getLastRecord();

$date = $new_version->getDate();

if ((isset($_POST['sqlState'])) && ($_POST['sqlState'] == 'NewRecord')) {
	$new_version ->setVersionId(trim($_POST['txtID']));
	$new_version ->setVersionName(trim($_POST['txtName']));
	$new_version ->setDateEntered($date);
	$new_version ->setCreatedUser(trim($_SESSION['user']));
	$new_version ->setFileVersion(trim($_POST['cmbFileVersion']));
	$new_version ->setdbVersion(trim($_POST['cmbdbVersion']));
	$new_version ->setDescription(trim($_POST['txtDescription']));	
	
	$message = $new_version->addVersions();
	if ($message) { 
		...
		$bugcode = $_GET['bugcode'];
		$pageID = $_GET['pageid'];
		header("Location: ./versions.php?message=$showMsg&captureState=AddMode");
	}	
	...
	$new_version ->setVersionName(trim($_POST['txtName']));
	$new_version ->setDateEntered($date);
	$new_version ->setModifiedUser(trim($_SESSION['user']));
	$new_version ->setFileVersion(trim($_POST['cmbFileVersion']));
	$new_version ->setdbVersion(trim($_POST['cmbdbVersion']));
	$new_version ->setDescription(trim($_POST['txtDescription']));	
	$new_version ->setDeleted(isset($_POST['chkDeleted'])?'1':'0');
	
	$message=$new_version->updateVersions();
...
function goBack() {
	...
	function addSave() {
		...
		document.frmVersions.pageID.value = "<?=$_GET['pageID']?>";
...
<form name="frmVersions" method="post" action="./versions.php?pageid=<?=$_GET['pageID']?>&bugcode=<?=$_GET['bugcode']?>">
<input type="hidden" name="pageID" value="">
  ...
  </tr><td width="177"><form name="frmVersions" method="post" action="./versions.php">
</table>
...
<label>
  ...
  <td><select name="cmbdbVersion">
    <?  $db = $new_version->getdbVersionList();
  ...
  <td><select name="cmbFileVersion">
    <?  $file = $new_version->getFileVersionList();
...
<? } else if ((isset($_GET['capturemode'])) && ($_GET['capturemode'] == 'updatemode')) {
	 $message = $new_version->filterVersions($_GET['id']);
?>
...
<table border="0" cellpadding="0" cellspacing="0">
 ...
 <tr> 
   ...
   <tr> 
    <td>Name</td>
    <td><input type="text" disabled readonly= "true" name="txtName" value=<?=$message[0][1]?>></td>  
  </tr>
  <tr>
  <td>DB Version</td>
