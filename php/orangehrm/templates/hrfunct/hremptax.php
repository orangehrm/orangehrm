...
<?php if(isset($this->getArr['capturemode']) && $this->getArr['capturemode'] == 'updatemode') { ?>
	...
	<table onclick="setUpdate(18)" onkeypress="setUpdate(18)" height="250" border="0" cellpadding="0" cellspacing="2">
<?php
    ...
    <tr>
		...
		<td><?php echo $lang_hrEmpMain_TaxExemptions; ?></td>
  		<td><input type="text" size="5" <?php echo $disabled;?> name="taxFederalExceptions" id="taxFederalExceptions"
  				value="<?php echo (isset($this->postArr['taxFederalExceptions'])) ? $this->postArr['taxFederalExceptions']:$editTaxInfo['tax_federal_exceptions'];?>">
  		</td>
	</tr>
    <tr>
		...
		<?php
			...
			for($c=0; $usStateList && count($usStateList)>$c ;$c++) {
				if($prevTaxState == $usStateList[$c][1]) {
			    	echo "<option selected value='" . $usStateList[$c][1] . "'>" . $usStateList[$c][2] . "</option>";
				} else {
		...
		<td><input type="text" size="5" <?php echo $disabled;?> name="taxStateExceptions" id="taxStateExceptions"
			value="<?php echo (isset($this->postArr['taxStateExceptions']))?$this->postArr['taxStateExceptions']:$editTaxInfo['tax_state_exceptions'];?>">
		</td>
		</tr>
		<tr>
