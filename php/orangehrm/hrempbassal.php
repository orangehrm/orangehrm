...
require_once OpenSourceEIM . '/lib/Confs/sysConf.php';
	...
	$empinfo = new EmpInfo();
	$einfo = $empinfo->filterEmpMain($_GET['id']);
	$empdet = $empinfo->filterEmpJobInfo($_GET['id']);
	$salGrd = $empdet[0][5];
	$empbassal = new EmpBasSalary();

if(isset($_POST['STAT']) && $_POST['STAT']=='ADD')
   {
	$empbassal->setEmpId($_GET['id']);
	$empbassal->setEmpSalGrdCode($salGrd);
	$empbassal->setEmpCurrCode($_POST['cmbCurrCode']);
	$empbassal->setEmpBasSal(trim($_POST['txtBasSal']));
...
function addCur()
{
var cntrl=document.frmEmpBasSal.cmbCurrCode;
document.frmEmpBasSal.action = document.frmEmpBasSal.action + "&CMB1=" + cntrl.value ;
document.frmEmpBasSal.submit();
}
...
<table width='100%' cellpadding='0' cellspacing='0' border='0' class='moduleTitle'>
  <tr>
    <td valign='top'>&nbsp; </td>
    <td width='100%'><h2>Renumeration</h2></td>
...
<form name="frmEmpBasSal" method="post" action="./hrempbassal.php?reqcode=<?=$_GET['reqcode']?>&id=<?=$_GET['id']?>">
                ...
                <tr>
                  ...
                  <td><table width="100%" border="0" cellpadding="5" cellspacing="0" class="">
                    <tr>
    				  ...
    				  <td width="50">&nbsp;</td>
					  <td>Surname</td>
						<td width="300"><font color="#204242"><strong><?=$einfo[0][3]?></strong></font></td>
					</tr>
					  <tr> 
						<td>Calling Name</td>
    				  ...
    				  <td width="50">&nbsp;</td>
						<td>Initials</td>
						<td><font color="#204242"><strong><?=$einfo[0][5]?></</font></td>
					  </tr>
                  </table></td>
...
if(isset($_GET['GRD']))
{
    ...
    $arr[1]=$_GET['GRD'];
    $arr[2]=$_GET['CID'];
    $edit=$empbassal->filterEmpBasSal($arr);
?>
                  ...
                  <td><table width="100%" border="0" cellpadding="5" cellspacing="0" class="">
                    <tr>
					  ...
					  <tr> 
						...
						<strong>
<?
						$currlist=$empbassal->getCurrCodes($edit[0][1]);
...
<?
						$curlist=$empbassal->getUnAssCurrCodes($salGrd,$_GET['id']);
						for($c=0;$curlist && count($curlist)>$c;$c++)
						    if(isset($_GET['CMB1']) && $_GET['CMB1']==$curlist[$c][2])
...
<?
$rset = $empbassal->getAssEmpBasSal($_GET['id']);
$currlist=$empbassal->getAllCurrCodes();
        ...
        echo '<tr>';
			...
			for($a=0;count($currlist)>$a;$a++)
			    if($currlist[$a][0]==$rset[$c][2])
				   $fname=$currlist[$a][1];
            echo "<td><a href='./hrempbassal.php?reqcode=" . $_GET['reqcode'] . "&id=" . $_GET['id']. "&GRD=" . $rset[$c][1] . "&CID=" . $rset[$c][2] . "'>" . $fname . "</a></td>";
            echo '<td>' . $rset[$c][3] .'</td>';
        echo '</tr>';
        }
