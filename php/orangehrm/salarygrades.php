...
require_once OpenSourceEIM . '/lib/Confs/sysConf.php';
	...
	$parent_salgrade = new SalaryGrades();
	$sysConst = new sysConf(); 
	$locRights=$_SESSION['localRights'];

	$lastRecord = $parent_salgrade ->getLastRecord();
	
if ((isset($_POST['sqlState'])) && ($_POST['sqlState'] == 'NewRecord')) {
	
	$parent_salgrade -> setSalGrdId(trim($_POST['txtSalGrdID']));
	$parent_salgrade -> setSalGrdDesc(trim($_POST['txtSalGrdDesc']));
	$message = $parent_salgrade ->addSalaryGrades();
	...
	if ($message) { 
		...
		$pageID = $_POST['pageID'];
		header("Location: ./salcurrdet.php?id=" .$_POST['txtSalGrdID']. "&pageid=" .$_GET['pageID']. "&uniqcode=".$_GET['uniqcode']);
		...
		$uniqcode = $_GET['uniqcode'];
		$pageID = $_GET['pageid'];
		header("Location: ./salarygrades.php?msg=$showMsg&capturemode=addmode&uniqcode=$uniqcode&pageID=$pageID");
	}
	...
	$parent_salgrade -> setSalGrdId(trim($_POST['txtSalGrdID']));
	$parent_salgrade -> setSalGrdDesc(trim($_POST['txtSalGrdDesc']));
	$message = $parent_salgrade ->updateSalaryGrades();
	...
	if ($message) { 
		...
		$pageID = $_GET['pageid'];
		$id = $_GET['id'];
		header("Location: ./salarygrades.php?id=$id&msg=$showMsg&capturemode=addmode&uniqcode=$uniqcode&pageID=$pageID");
	}
...
<script>			
	...
	function addSave() {
		...
		document.frmSalGrd.pageID.value = "<?=$_GET['pageID']?>";
...
<table width='100%' cellpadding='0' cellspacing='0' border='0' class='moduleTitle'>
  <tr>
    <td valign='top'></td>
    <td width='100%'><h2>Salary Grade : Job Information</h2></td>
...
<form name="frmSalGrd" method="post" action="./salarygrades.php?pageid=<?=$_GET['pageID']?>&uniqcode=<?=$_GET['uniqcode']?>">
<input type="hidden" name="pageID" value="">
  ...
  </tr><td width="177"><form name="frmSalGrd" method="post" action="./salarygrades.php">
</table>
                ...
                <tr>
                  ...
                  <td><table width="100%" border="0" cellpadding="5" cellspacing="0" class="">
						  <tr> 
						    ...
						    <td> <input type="text" readonly="true" name="txtSalGrdID" value="<?=$lastRecord?>">
						  </tr>
						  <tr> 
...
<? } else if ((isset($_GET['capturemode'])) && ($_GET['capturemode'] == 'updatemode')) {
	 $message = $parent_salgrade ->filterSalaryGrades($_GET['id']);
?>
           ...
           <table border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  ...
                  <td><table width="100%" border="0" cellpadding="5" cellspacing="0" class="">
							  <tr> 
							    <td><strong>Code</strong></td>
							    <td> <input type="text" readonly="true" name="txtSalGrdID" value=<?=$_GET['id'] ?> >
							    <td>&nbsp;</td>
							  </tr>
							  <tr> 
							    <td><strong>Description</strong></td>
					  ...
					  <tr><td><a href="./salcurrdet.php?id=<?=$_GET['id']?>&pageid=<?=$_GET['pageID']?>&uniqcode=<?=$_GET['uniqcode']?>">Add Currency Detail</a></td>
