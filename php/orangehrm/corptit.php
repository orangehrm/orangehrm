...
require_once OpenSourceEIM . '/lib/Confs/sysConf.php';
	...
	$parent_corptit = new CorpTitle();
	$sysConst = new sysConf(); 
	$locRights=$_SESSION['localRights'];

	$lastRecord = $parent_corptit ->getLastRecord();
	
if ((isset($_POST['sqlState'])) && ($_POST['sqlState'] == 'NewRecord')) {
	
		$parent_corptit -> setCtId(trim($_POST['txtCorpID']));
		$parent_corptit -> setCtDesc(trim($_POST['txtCorpDesc']));
		$parent_corptit -> setCtTopLev(isset($_POST['chkCorpTopLev'])?'1':'0');
		$parent_corptit -> setCtHead(isset($_POST['chkCorpHead'])?'1':'0');
		$parent_corptit -> setCtNxtUpg(trim($_POST['cmbCorpNxtUpg']));
		$parent_corptit -> setCtHeadCnt(trim($_POST['txtCorpHeadCount']));
		$parent_corptit -> setCtSalGrdId(trim($_POST['cmbCorpSalGrd']));
		$parent_corptit -> setCtRevDat(null);
		$parent_corptit -> setCtHirachId(null);
        $message = $parent_corptit ->addCorpTitles();
	...
	if ($message) { 
		...
		$uniqcode = $_GET['uniqcode'];
		$pageID = $_GET['pageID'];
		header("Location: ./corptit.php?msg=$showMsg&capturemode=addmode&uniqcode=$uniqcode&pageID=$pageID");
	}
		...
		$parent_corptit -> setCtRevDat(null);
		$parent_corptit -> setCtHirachId(null);
        $message = $parent_corptit ->updateCorpTitles();
	...
	if ($message) { 
		...
		$pageID = $_GET['pageID'];
		$id = $_GET['id'];
		header("Location: ./corptit.php?msg=$showMsg&id=$id&capturemode=updatemode&uniqcode=$uniqcode&pageID=$pageID");
	}
...
function cntField() {
	...
	function addSave() {
		...
		document.frmCorpTitles.pageID.value = "<?=$_GET['pageID']?>";
...
<table width='100%' cellpadding='0' cellspacing='0' border='0' class='moduleTitle'>
  <tr>
    <td valign='top'> </td>
    <td width='100%'><h2>Corporate Titles : Job Information</h2></td>
...
<form name="frmCorpTitles" method="post" action="./corptit.php?pageid=<?=$_GET['pageID']?>&uniqcode=<?=$_GET['uniqcode']?>">
<input type="hidden" name="pageID" value="">
  ...
  </tr><td width="177"><form name="frmCorpTitles" method="post" action="./corptit.php">
</table>
                ...
                <tr>
                  ...
                  <td><table width="100%" border="0" cellpadding="5" cellspacing="0" class="">
						  <tr> 
						    ...
						    <td> <input type="text" readonly="true" name="txtCorpID" value="<?=$lastRecord; ?>">
						  </tr>
						<tr>
						  ...
						  <tr>
						    <td><strong>Top in Hierachy</strong></td>
						    <td> <input type="checkbox" name="chkCorpTopLev" value="1">
						    </td>
						  </tr>
						  <tr>
						    <td><strong>Multiple Heads</strong></td>
						  ...
						  <tr>
						    <td><strong>Next Level Upgrade</strong></td>
						    <td> <select name="cmbCorpNxtUpg">
						    		<option value="0">-Select Corp. Title-</option>
						    <?  $corptitles= $parent_corptit->getCorpTitles();
						  ...
						  <tr>
						    ...
						    <?  $salgrds= $parent_corptit->getSalGrades();
...
<? } else if ((isset($_GET['capturemode'])) && ($_GET['capturemode'] == 'updatemode')) {
	 $message = $parent_corptit ->filterCorpTitles($_GET['id']);
?>
           ...
           <table border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  ...
                  <td><table width="100%" border="0" cellpadding="5" cellspacing="0" class="">
							  <tr> 
							    <td><strong>Code</strong></td>
							    <td> <input type="text" readonly="true" name="txtCorpID" value="<?=$_GET['id']?>">
							  </tr>
							<tr>
							    <td><strong>Description</strong></td>
