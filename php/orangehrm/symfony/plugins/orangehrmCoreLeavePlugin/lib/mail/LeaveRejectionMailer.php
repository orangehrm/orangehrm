...
class LeaveRejectionMailer extends orangehrmLeaveMailer {
    ...
    public function sendToApplicant() {
        ...
        if (!empty($to)) {
            ...
            try {
                ...
                $message = new LeaveRejectionMailContent($this->performer, $this->recipient, $this->leaveRequest, $this->leaveList, $this->requestType);
                ...
                $logMessage = "Leave rejection email was sent to $to";
                $this->logResult('Success', $logMessage);

            } catch (Exception $e) {

                $logMessage = "Couldn't send leave rejection email to $to";
    ...
    public function sendToSubscribers() {
        ...
        $subscription = $mailNotificationService->getSubscription(MailNotification::LEAVE_REJECTION);
        ...
        if ($subscription instanceof MailNotification) {
            ...
            if ($subscription->getStatus() == MailNotification::STATUS_SUBSCRIBED) {
                ...
                try {
                    ...
                    $logMessage = "Leave rejection subscription email was sent to $to";
                    $this->logResult('Success', $logMessage);

                } catch (Exception $e) {

                    $logMessage = "Couldn't send leave rejection subscription email to $to";
                    $logMessage .= '. Reason: '.$e->getMessage();
                    $this->logResult('Failure', $logMessage);
