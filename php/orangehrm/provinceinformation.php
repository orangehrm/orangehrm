...
if ((isset($_POST['sqlState'])) && ($_POST['sqlState'] == 'NewRecord')) {
	$parent_provinceinfo -> setProvinceInfoId($parent_provinceinfo ->getLastRecord());
	$parent_provinceinfo -> setProvinceInfoDesc(trim($_POST['txtProvinceInfoDesc']));
	$parent_provinceinfo -> setCountryId(trim($_POST['selCountryId']));
	$message = $parent_provinceinfo ->addProvinceInfo();
	...
	if ($message) { 
		...
		$uniqcode = $_GET['uniqcode'];
		$pageID = $_GET['pageid'];
		header("Location: ./provinceinformation.php?message=$showMsg&captureState=AddMode");
	}
	
} else if ((isset($_POST['sqlState'])) && ($_POST['sqlState'] == 'UpdateRecord')) {
	
	$parent_provinceinfo -> setProvinceInfoId(trim($_POST['txtProvinceInfoId']));
	$parent_provinceinfo -> setProvinceInfoDesc(trim($_POST['txtProvinceInfoDesc']));
	$parent_provinceinfo -> setCountryId(trim($_POST['selCountryId']));
	$message = $parent_provinceinfo ->updateProvinceInfo();
...
<?
	...
	$lastRecord = $parent_provinceinfo ->getLastRecord();
?>
...
return flag;
	...
	function addSave() {
		var txt=document.frmProvinceInformation.txtProvinceInfoDesc;
		if (!alpha(txt)) {
		...
		document.frmProvinceInformation.pageID.value = "<?=$_GET['pageID']?>";
		document.frmProvinceInformation.submit();		
	}
	
	function clearAll() {
		document.frmProvinceInformation.txtProvinceInfoDesc.value = '';
...
<table width='100%' cellpadding='0' cellspacing='0' border='0' class='moduleTitle'>
  <tr>
    <td valign='top'> </td>
    <td width='100%'><h2>State Information : Geo Information</h2></td>
...
<form name="frmProvinceInformation" method="post" action="./provinceinformation.php?pageid=<?=$_GET['pageID']?>&uniqcode=<?=$_GET['uniqcode']?>">
                ...
                <tr>
                  ...
                  <td><table width="100%" border="0" cellpadding="5" cellspacing="0" class="">
							  <tr> 
							    ...
							    <td> <input type="text" readonly="true" name="txtProvinceInfoId" value=<?=$lastRecord; ?> ></td>
							  </tr>
							  <tr> 
							    <td><strong>Description</strong></td>
							    <td> <textarea name='txtProvinceInfoDesc' rows="3" tabindex='3' cols="30"></textarea></td>
							  </tr>
							  <tr> 
							    <td><strong>Country</strong></td>
							    <td> <select name="selCountryId"> 
							    		<option value="0">-Select Country-</option>
							    <? $getResultSet = $parent_provinceinfo->getCountryCodes(); 
...
<? } else if ((isset($_GET['capturemode'])) && ($_GET['capturemode'] == 'updatemode')) {
	 $message = $parent_provinceinfo ->filterProvinceInfo($_GET['id']);
?>
           ...
           <table border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  ...
                  <td><table width="100%" border="0" cellpadding="5" cellspacing="0" class="">
						  <tr> 
						    <td><strong>Code</strong></td>
						    <td> <input type="text" readonly="true" name="txtProvinceInfoId" value=<?=$_GET['id'] ?> ></td>
						  </tr>
						  <tr> 
						    <td><strong>Description</strong></td>
						  	<td align="left" valign="top"> <textarea name='txtProvinceInfoDesc' disabled rows="3" tabindex='3' cols="30"><?=$message[0][1]?></textarea></td>
						  </tr>
						    <td><strong>Country Information</strong></td>
