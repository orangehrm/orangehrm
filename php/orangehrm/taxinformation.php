...
require_once OpenSourceEIM . '/lib/Confs/sysConf.php';
	...
	$parent_taxinformation = new TaxInfo();
	$sysConst = new sysConf(); 
	$locRights=$_SESSION['localRights'];

	$lastRecord = $parent_taxinformation ->getLastRecord();
	
if ((isset($_POST['sqlState'])) && ($_POST['sqlState'] == 'NewRecord')) {
	$parent_taxinformation -> setTaxId(trim($_POST['txtTaxInfoId']));
	$parent_taxinformation -> setTaxDesc(trim($_POST['txtTaxInfoDesc']));
	$message = $parent_taxinformation ->addTaxInfo();
	...
	if ($message) { 
		...
		$uniqcode = $_GET['uniqcode'];
		$pageID = $_GET['pageid'];
		header("Location: ./taxinformation.php?message=$showMsg&captureState=AddMode");
	}
	...
	$parent_taxinformation -> setTaxId(trim($_POST['txtTaxInfoId']));
	$parent_taxinformation -> setTaxDesc(trim($_POST['txtTaxInfoDesc']));
	$message = $parent_taxinformation ->updateTaxInfo();
...
<script>			
	...
	function addSave() {
		...
		if (document.frmTaxInformation.txtTaxInfoDesc.value == '') {
			alert ("Description Cannot be a Blank Value!");
			return false;
		}
		
		document.frmTaxInformation.sqlState.value = "NewRecord";
		document.frmTaxInformation.pageID.value = "<?=$_GET['pageID']?>";
		document.frmTaxInformation.submit();		
	}
	
	function clearAll() {
		document.frmTaxInformation.txtTaxInfoDesc.value = '';
	}
...
<table width='100%' cellpadding='0' cellspacing='0' border='0' class='moduleTitle'>
  <tr>
    <td valign='top'> </td>
    <td width='100%'><h2>Tax : Tax Information</h2></td>
...
<form name="frmTaxInformation" method="post" action="./taxinformation.php?pageid=<?=$_GET['pageID']?>&uniqcode=<?=$_GET['uniqcode']?>">
                ...
                <tr>
                  ...
                  <td><table width="100%" border="0" cellpadding="5" cellspacing="0" class="">
						  <tr> 
						    ...
						    <td> <input type="text" readonly="true" name="txtTaxInfoId" value=<?=$lastRecord; ?> ></td>
						  </tr>
						  <tr> 
						    <td><strong>Description</strong></td>
						    <td> <textarea name='txtTaxInfoDesc' rows="3" tabindex='3' cols="30"></textarea></td>
						  </tr>
...
<? } else if ((isset($_GET['capturemode'])) && ($_GET['capturemode'] == 'updatemode')) {
	 $message = $parent_taxinformation ->filterTaxInfo($_GET['id']);
?>
...
function edit()
{
	...
	var frm=document.frmTaxInformation;
//  alert(frm.elements.length);
	...
	function addUpdate() {
		...
		document.frmTaxInformation.sqlState.value = "UpdateRecord";
		document.frmTaxInformation.pageID.value = "<?=$_GET['pageID']?>";
		document.frmTaxInformation.submit();		
	}
           ...
           <table border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  ...
                  <td><table width="100%" border="0" cellpadding="5" cellspacing="0" class="">
							  <tr> 
							    <td><strong>Code</strong></td>
							    <td> <input type="text" readonly="true" name="txtTaxInfoId" value=<?=$_GET['id'] ?> ></td>
							  </tr>
							  <tr> 
							    <td><strong>Description</strong></td>
							  	<td> <textarea name='txtTaxInfoDesc' rows="3" tabindex='3' disabled cols="30"><?=$message[0][1]?></textarea></td>
							  </tr>
