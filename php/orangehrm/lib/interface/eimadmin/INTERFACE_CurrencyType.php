...
class INTERFACE_CurrencyType {
	
	function INTERFACE_CurrencyType() {
	...
	function parseData($postArr) {	
		if (($postArr['sqlState'] == 'NewRecord') && $this->locRights['add']) {
			...
			$message = $this->parent_currencytype ->addCurrencyTypes();
			...
			if ($message) { 
				...
				$uniqcode = $_GET['uniqcode'];
				$pageID = $postArr['pageID'];
				header("Location: ./currencytypes.php?msg=$showMsg&capturemode=addmode&uniqcode=$uniqcode&pageID=$pageID");
			}
			...
			$this->parent_currencytype -> setCurrencyDescription(trim($postArr['txtCurrencyTypeDesc']));
			$message = $this->parent_currencytype ->updateCurrencyTypes();
			...
			if ($message) { 
				...
				$pageID = $postArr['pageID'];
				$id = $_GET['id'];
				header("Location: ./currencytypes.php?msg=$showMsg&id=$id&capturemode=updatemode&uniqcode=$uniqcode&pageID=$pageID");
			}
	...
	function editData($id) {
	 $message = $this->parent_currencytype ->filterCurrencyTypes($id);
