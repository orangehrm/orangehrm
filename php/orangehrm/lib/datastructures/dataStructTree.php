...
class dataStructTree {
	...
	function addNode ($rgt, $addStr, $strDesc, $addParnt, $location) {
		...
		$sqlString1=sprintf("UPDATE hs_hr_compstructtree SET rgt=rgt+2 WHERE rgt>%d", $rgt-1);

		$sqlString2=sprintf("UPDATE hs_hr_compstructtree SET lft=lft+2 WHERE lft>%d", $rgt-1);

		$sqlString3=sprintf("INSERT INTO hs_hr_compstructtree SET lft=%d, rgt=%d, title='%s', Description='%s', parnt=%d, loc_code='%s'", $rgt, $rgt+1, $addStr, $strDesc, $addParnt, $location);
	...
	function removeNode ( $lft, $rgt ) {
		...
		$change=($rgt - $lft + 1); // Change in left and rignt from the delete. Think a bit!

		

		$sqlString1=sprintf("DELETE FROM hs_hr_compstructtree WHERE rgt<%d AND lft>%d", $rgt+1, $lft-1);		

		$sqlString2=sprintf("UPDATE hs_hr_compstructtree SET lft=lft-%d WHERE lft>%d", $change, $lft-1);

		$sqlString3=sprintf("UPDATE hs_hr_compstructtree SET rgt=rgt-%d WHERE rgt>%d", $change, $rgt-1);		
	...
	function editNode($id, $title, $location, $strDesc) {		

		

		$sqlString1=sprintf("UPDATE hs_hr_compstructtree SET title='%s', Description='%s', loc_code='%s' WHERE ID=%d", $title, $strDesc, $location, $id);
