...
class ViewController {
    ...
    private $activityHaveTimeItems = false;// Should only set in PAC
    private $customersHaveTimeItems = false; // Should only set in CUS
    ...
    function viewList($getArr, $postArr) {
        ...
        switch ($getArr['uniqcode']) {
            ...
            default:
                ...
                if ($this->customersHaveTimeItems) {
                    $form_creator->popArr['customersHaveTimeItems'] = true;
                } else {
                    $form_creator->popArr['customersHaveTimeItems'] = false;
                }
                
                    if ($this->activityHaveTimeItems) {
                    $form_creator->popArr['activitiesHaveTimeItems'] = true;
                } else {
                    $form_creator->popArr['activitiesHaveTimeItems'] = false;
                }
    ...
    function delParser($indexCode, $arrList) {
        ...
        switch ($this->indexCode) {
            ...
            case 'CUS':
                ...
                if ($this->customers->haveTimeItems($arrList[0])) {
                    $this->customersHaveTimeItems = true;
                }
                $res = false;
                if ($token == $_POST['token'] && !$this->customersHaveTimeItems) {
            ...
            case 'PAC': // Project activity
                ...
                if (ProjectActivity::haveTimeItems($arrList[0])) {
                    $this->activityHaveTimeItems = true;
                }

                $authorizeObj = new authorize($_SESSION['empID'], $_SESSION['isAdmin']);
                $projectId = $_GET['projectId'];
                $res = false;
                if (($authorizeObj->isAdmin() || $authorizeObj->isProjectAdminOf($projectId)) && !$this->activityHaveTimeItems) {
