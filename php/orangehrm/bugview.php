...
require_once OpenSourceEIM . '/lib/Controllers/BugViewController.php';
$srchlist[0] = array( 0 , 1 , 2 );
$srchlist[1] = array( ' ' , 'ID' , 'Description' );

	$viewcontroller = new BugViewController();	
...
if ((isset($_GET['bugcode'])) && ($_GET['bugcode'] != '')) {

	$pageInfo = $viewcontroller -> getPageID(trim($_GET['bugcode']));
 	$headingInfo = $viewcontroller -> getHeadingInfo(trim($_GET['bugcode']));
}

if (isset($_POST['delState'])&& ($_POST['delState']=="DeleteMode")) {
    $arrList[0]=$_POST['chkLocID'];

    $viewcontroller->delParser(trim($_GET['bugcode']),$arrList);
    }
...
if (isset($_POST['captureState'])&& ($_POST['captureState']=="SearchMode")) {
    ...
    $message = $viewcontroller -> getInfo(trim($_GET['bugcode']),$currentPage,$strName,$choice);
    } else 
		$message = $viewcontroller -> getInfo(trim($_GET['bugcode']),$currentPage);
...
<script>		
	...
	function returnAdd() {
		...
		location.href = "<?=$pageInfo?>" + ".php?sqlmode=addmode&pageID=<?=$pageInfo?>&bugcode=<?=$_GET['bugcode']?>&capturemode=addmode";
...
<form name="standardView" method="post" action="./bugview.php?bugcode=<?=$_GET['bugcode']?>">
  </td>
  <td width='100%'><h2> 
      <?=$headingInfo[3]?>
    </h2></td>
...
<?
if (isset($_POST['captureState'])&& ($_POST['captureState']=="SearchMode")) 				
    $temp = $viewcontroller -> countList(trim($_GET['bugcode']),$strName,$choice);
else 
    $temp = $viewcontroller -> countList(trim($_GET['bugcode']));
...
?> 
		...
		<? 		}  ?>
				  ...
				  <td width="250"><a href="<?=$pageInfo?>.php?id=<?=$message[$j][0]?>&sqlmode=addmode&bugcode=<?=$_GET['bugcode']?>&capturemode=updatemode&pageID=<?=$pageInfo?>" class="listViewTdLinkS1"><?=$message[$j][0]?></a></td>
		...
		<? 		}  ?>
				  <td bgcolor="#EEEEEE" width="250"><a href="<?=$pageInfo?>.php?id=<?=$message[$j][0]?>&sqlmode=addmode&bugcode=<?=$_GET['bugcode']?>&capturemode=updatemode&pageID=<?=$pageInfo?>" class="listViewTdLinkS1"><?=$message[$j][0]?></a></td>
