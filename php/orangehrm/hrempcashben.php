...
$bfilter = array('Salary Point' => 1,'Designation' => 2,'Employee' => 3);
...
if(isset($_POST['STAT']) && $_POST['STAT']=='ADD')
   {
   $arr=$_POST['chkdel'];
   $benlist=$empcashben->getBenCodes();
   
   for($c=0;count($arr)>$c;$c++)
       if($arr[$c]!=NULL)
       {
       	$empcashben->setEmpId($_GET['id']);
       	$empcashben->setEmpCashBenCode($arr[$c]);
       	for($j=0;count($benlist)>$j;$j++)
       	    if($benlist[$j][0]==$arr[$c])
       	    	$empcashben->setEmpBenAmount($benlist[$j][2]);
       	$empcashben->setEmpBenDatAss(date('Y-m-d'));
       	$empcashben->setEmpBenFltType($bfilter['Employee']);
       	$empcashben->addUnAssEmpCashBen();
       }
   }

if(isset($_POST['STAT']) && $_POST['STAT']=='ADDOTH')
   {
   $arr=$_POST['chkdel'];
   $benlist=$empcashben->getBenCodes();
...
if(isset($_POST['STAT']) && $_POST['STAT']=='EDIT')
   {
    $empcashben->setEmpId($_GET['id']);
	$empcashben->setEmpCashBenCode($_POST['cmbCashBenCode']);
	$empcashben->setEmpBenAmount($_POST['txtBenAmount']);
	$empcashben->setEmpBenDatAss($_POST['txtBenDatAss']);
...
function edit()
{
	...
	var frm=document.frmEmpCashBen;
...
{
      var check = 0;
		with (document.frmAddEmpCashBen) {
    ...
    document.frmAddEmpCashBen.STAT.value="ADD";
    document.frmAddEmpCashBen.submit();
}

function addOthEXT()
{
      var check = 0;
		with (document.frmOthEmpCashBen) {
			for (var i=0; i < elements.length; i++) {
				if ((elements[i].type == 'checkbox') && (elements[i].checked == true)){
					check = 1;
				}
    ...
    document.frmOthEmpCashBen.STAT.value="ADDOTH";
    document.frmOthEmpCashBen.submit();
}

function editEXT()
{
var cnt=document.frmEmpCashBen.txtBenAmount;
  ...
  document.frmEmpCashBen.STAT.value="EDIT";
  document.frmEmpCashBen.submit();
}

function delEXT()
{
      var check = 0;
		with (document.frmDelEmpCashBen) {
			for (var i=0; i < elements.length; i++) {
				if ((elements[i].type == 'checkbox') && (elements[i].checked == true)){
					check = 1;
				}
    ...
    document.frmDelEmpCashBen.STAT.value="DEL";
    document.frmDelEmpCashBen.submit();
}
...
<table width='100%' cellpadding='0' cellspacing='0' border='0' class='moduleTitle'>
  <tr>
    <td valign='top'>&nbsp; </td>
    <td width='100%'><h2>Cash Benefits</h2></td>
...
<form name="frmEmpCashBen" method="post" action="./hrempcashben.php?reqcode=<?=$_GET['reqcode']?>&id=<?=$_GET['id']?>">
...
if(isset($_GET['BEN']))
{
    $arr[0]=$_GET['id'];
    $arr[1]=$_GET['BEN'];
    $edit=$empcashben->filterEmpCashBen($arr);
?>
...
<br><br>
      <table border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  ...
                  <td><table width="100%" border="0" cellpadding="5" cellspacing="0" class="">
					  <tr> 
						<td valign="top">Cash Benefit</td>
						<td align="left" valign="top"><input type="hidden" name="cmbCashBenCode" value="<?=$edit[0][1]?>"><strong>
<?
						$benlist=$empcashben->getBenCodes();
...
?>						
					  ...
					  <tr> 
						<td valign="top">Benefit Amount</td>
						<td align="left" valign="top"><input type="text" name="txtBenAmount" disabled value="<?=$edit[0][2]?>">
						</td>
					  </tr>
					  <tr> 
						<td valign="top">Date Assigned</td>
						<td align="left" valign="top"><input type="text" name="txtBenDatAss" readonly disabled value=<?=$edit[0][3]?>>&nbsp;<input class="button" disabled type="button" value=".." onclick="if(self.gfPop)gfPop.fPopCalendar(document.frmEmpCashBen.txtBenDatAss);return false;">
						</td>
					  </tr>
					  <tr> 
						<td valign="top">Filter Type</td>
						<? $flt=array_keys($bfilter); ?>
						<td align="left" valign="top"><strong><?=$flt[((int)$edit[0][4]-1)]?></strong></td>
					  </tr>
					  <tr> 
						<td valign="top"></td>
      ...
      <table border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  ...
                  <td><table width="100%" border="0" cellpadding="5" cellspacing="0" class="">
					  ...
					  <tr> 
						<td valign="top">Benefit Amount</td>
						<td align="left" valign="top"><input type="text" disabled >
						</td>
					  </tr>
					  <tr> 
						<td valign="top">Date Assigned</td>
...
<tr>
<td valign="top">
<form name="frmDelEmpCashBen" method="post" action="./hrempcashben.php?reqcode=<?=$_GET['reqcode']?>&id=<?=$_GET['id']?>">
...
<td><a href="./hrempcashben.php?reqcode=<?=$_GET['reqcode']?>&id=<?=$_GET['id']?>&OBEN=1">Add Other Benefits</a></td>
  </tr>
<tr><td>&nbsp;</td></tr>
</table>
      <table border="0" cellpadding="0" cellspacing="0">
...
<?
$rset = $empcashben->getAssEmpCashBen($_GET['id']);
$benlist=$empcashben->getBenCodes();
        ...
        echo '<tr>';
			...
			for($a=0;count($benlist)>$a;$a++)
			    if($benlist[$a][0]==$rset[$c][1])
				   $fname=$benlist[$a][1];
            echo "<td><a href='./hrempcashben.php?reqcode=" . $_GET['reqcode'] . "&id=" . $_GET['id']. "&BEN=" . $rset[$c][1] . "'>" . $fname . "</a></td>";
            echo '<td>' . $rset[$c][2] .'</td>';
        echo '</tr>';
        }
...
<td valign="top">
<form name="frmAddEmpCashBen" method="post" action="./hrempcashben.php?reqcode=<?=$_GET['reqcode']?>&id=<?=$_GET['id']?>">
<table width='100%' cellpadding='0' cellspacing='0' border='0'>
  <tr>
    <td valign='top'>&nbsp; </td>
    <td valign='top' align='right' nowrap style='padding-top:3px; padding-left: 5px;'></td>
...
<?
$rset = $empcashben->getUnAssEmpCashBen($_GET['id']);
$benlist=$empcashben->getBenCodes();
...
<tr>
<td>
<form name="frmOthEmpCashBen" method="post" action="./hrempcashben.php?reqcode=<?=$_GET['reqcode']?>&id=<?=$_GET['id']?>">
<? 		if(isset($_GET['OBEN'])) { ?>
<table width='100%' cellpadding='0' cellspacing='0' border='0'>
  <tr>
    <td valign='top'>&nbsp; </td>
...
<?
$rset = $empcashben->getOthEmpCashBen($_GET['id']);
