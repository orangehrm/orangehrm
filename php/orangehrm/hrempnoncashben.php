...
if(isset($_POST['STAT']) && $_POST['STAT']=='ADD')
   {
   $arr=$_POST['chkdel'];
   $benlist=$empnoncashben->getBenCodes();
   
   for($c=0;count($arr)>$c;$c++)
       if($arr[$c]!=NULL)
       {
       	$empnoncashben->setEmpId($_GET['id']);
       	$empnoncashben->setEmpBenCode($arr[$c]);
		$empnoncashben->setEmpBenIssDat(date('Y-m-d'));
		$empnoncashben->setEmpBenQty(1);
		$empnoncashben->setEmpBenComment(null);
       	for($j=0;count($benlist)>$j;$j++)
       	    if($benlist[$j][0]==$arr[$c])
       	    	$empnoncashben->setEmpBenItmReturnableFlag($benlist[$j][2]);
		$empnoncashben->setEmpBenItmRetDat(null);
		$empnoncashben->setEmpBenItmRetFlag(null);
		$empnoncashben->setEmpBenFltType($bfilter['Employee']);
...
if(isset($_POST['STAT']) && $_POST['STAT']=='EDIT')
   {
    $empnoncashben->setEmpId($_GET['id']);
	$empnoncashben->setEmpBenCode($_POST['cmbBenCode']);
	$empnoncashben->setEmpBenIssDat(trim($_POST['txtBenIssDat']));
	$empnoncashben->setEmpBenQty(trim($_POST['txtBenQty']));
	$empnoncashben->setEmpBenComment(trim($_POST['txtBenComment']));
	$empnoncashben->setEmpBenItmReturnableFlag(isset($_POST['chkBenItmReturnableFlag'])?'1':'0');
	$empnoncashben->setEmpBenItmRetDat(trim($_POST['txtBenItmRetDat']));
	$empnoncashben->setEmpBenItmRetFlag(isset($_POST['chkBenItmRetFlag'])?'1':'0');
...
<form name="frmEmpNonCashBen" method="post" action="./hrempnoncashben.php?reqcode=<?=$_GET['reqcode']?>&id=<?=$_GET['id']?>">
...
if(isset($_GET['BEN']))
{
    $arr[0]=$_GET['id'];
    $arr[1]=$_GET['BEN'];
    $edit=$empnoncashben->filterEmpNonCashBen($arr);
?>
...
<tr>
<td valign="top">
<form name="frmDelEmpNonCashBen" method="post" action="./hrempnoncashben.php?reqcode=<?=$_GET['reqcode']?>&id=<?=$_GET['id']?>">
<input type="hidden" name="STAT" value="">
...
<? 	} ?>
  ...
  <td><a href="./hrempnoncashben.php?reqcode=<?=$_GET['reqcode']?>&id=<?=$_GET['id']?>&OBEN=1">Add Other Benefits</a></td>
  </tr>
<tr><td>&nbsp;</td></tr>
</table>
      <table border="0" cellpadding="0" cellspacing="0">
...
<?
$rset = $empnoncashben->getAssEmpNonCashBen($_GET['id']);
$benlist=$empnoncashben->getBenCodes();
        ...
        echo '<tr>';
			...
			for($a=0;count($benlist)>$a;$a++)
			    if($benlist[$a][0]==$rset[$c][1])
				   $fname=$benlist[$a][1];
            echo "<td><a href='./hrempnoncashben.php?reqcode=" . $_GET['reqcode'] . "&id=" . $_GET['id']. "&BEN=" . $rset[$c][1] . "'>" . $fname . "</a></td>";
            echo '<td>' . $rset[$c][3] .'</td>';
        echo '</tr>';
        }
...
<td valign="top">
<form name="frmAddEmpNonCashBen" method="post" action="./hrempnoncashben.php?reqcode=<?=$_GET['reqcode']?>&id=<?=$_GET['id']?>">
<table width='100%' cellpadding='0' cellspacing='0' border='0'>
  <tr>
    <td valign='top'>&nbsp; </td>
    <td valign='top' align='right' nowrap style='padding-top:3px; padding-left: 5px;'></td>
...
<?
$rset = $empnoncashben->getUnAssEmpNonCashBen($_GET['id']);
$benlist=$empnoncashben->getBenCodes();
...
<tr>
<td valign="top">
<form name="frmOthEmpNonCashBen" method="post" action="./hrempnoncashben.php?reqcode=<?=$_GET['reqcode']?>&id=<?=$_GET['id']?>">
<? if(isset($_GET['OBEN'])) { ?>
<table width='100%' cellpadding='0' cellspacing='0' border='0'>
  <tr>
    <td valign='top'>&nbsp; </td>
...
<?
$rset = $empnoncashben->getOthEmpNonCashBen($_GET['id']);
