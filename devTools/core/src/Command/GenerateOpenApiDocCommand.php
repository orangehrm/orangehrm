...
class GenerateOpenApiDocCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $textHelper = new TextHelperService();
        ...
        foreach (Config::get(Config::PLUGIN_PATHS) as $pluginAbsPath) {
            if ($textHelper->strContains($pluginAbsPath, 'orangehrmCorePlugin')) {
                continue;
            }
        ...
        $pos = strpos($openApiDefinition, '3.1.0');
        if ($pos !== false) {
        ...
        file_put_contents($buildDir . '/index.html', $swaggerUI);
