...
describe('Configuration - custom fields', function () {
  ...
  describe('Add a custom field and Duplicate record validations testing', function () {
      it('add a custom field ,check toast message and duplicate license', () => {
          ...
          cy.get(':nth-child(2) > .oxd-input-group > :nth-child(2) > .oxd-select-wrapper > .oxd-select-dropdown',
          ).contains('Personal Details').click();
          cy.get(':nth-child(2) > .oxd-grid-2 > .oxd-grid-item > .oxd-input-group > :nth-child(2) > .oxd-select-wrapper > .oxd-select-text').click();
          cy.get(':nth-child(2) > .oxd-grid-2 > .oxd-grid-item > .oxd-input-group > :nth-child(2) > .oxd-select-wrapper > .oxd-select-dropdown',)
    ...
    describe('Text field validation testing', function () {
        it('required field verification', () => {
            ...
            cy.get('.oxd-button--secondary').click();
            cy.get(':nth-child(1) > .oxd-grid-2 > .organization-name-container > .oxd-input-group > .oxd-text').should('include.text', 'Required');
            cy.get(':nth-child(2) > .oxd-input-group > .oxd-text').should('include.text', 'Required');
            cy.get(':nth-child(2) > .oxd-grid-2 > .oxd-grid-item > .oxd-input-group > .oxd-text').should('include.text', 'Required');
        });
        it('maximum length validation', () => {
            cy.get('.oxd-button').click();
            cy.get(':nth-child(2) > .oxd-input').type(charLength.chars250.text);
            cy.get('.oxd-button--secondary').click();
            cy.get(':nth-child(1) > .oxd-grid-2 > .organization-name-container > .oxd-input-group > .oxd-text').should(
              'include.text',
        ...
        it('Remaining Customfield count decrement', () => {
            ...
            cy.get('.orangehrm-custom-field-title > .oxd-text--p')
               ...
               .then((text) => {
                   var fullText = text;
                   var pattern = /[0-9]+/g;
                   var num = fullText.match(pattern);
                   var currentRemno = parseInt(num);
                   cy.get('.oxd-button').click();
                   ...
                   cy.visit('/pim/listCustomFields');
                   cy.get('.orangehrm-custom-field-title > .oxd-text--p').contains('Remaining number of custom fields:')
                    .invoke('text')
                    .then((text) => {
                        var fullText = text;
                        var pattern = /[0-9]+/g;
    ...
    describe('Updating an existing customfield', function () {
        it('Update Name of custom field and type', () => {
            ...
            cy.get(':nth-child(2) > .oxd-grid-2 > .oxd-grid-item > .oxd-input-group > :nth-child(2) > .oxd-select-wrapper > .oxd-select-dropdown',)
                .contains('Drop Down')
                .click();
            cy.get(':nth-child(2) > .oxd-input-group > :nth-child(2) > .oxd-input').type("1,2,3");
            cy.get('.oxd-button--secondary').click();
            cy.get('.oxd-toast')
                .should(
                 'include.text', 
    ...
    describe('Deleting existing customfields and check toast', function () {
        it('Delete a single customfield, check confirmation message and toast', () => {
            cy.get(':nth-child(1) > .oxd-table-row > [style="flex: 1 1 0%;"] > .oxd-table-cell-actions > :nth-child(1) > .oxd-icon').click();
    ...
    describe('Cancel button testing', function () {
        it('visiting add new customfield and clicking cancel', () => {
            ...
            cy.get('.orangehrm-custom-field-title > .oxd-text--h6').should('include.text', 'Custom Fields');
        });
    });
    ...
    describe('Verify Add button after adding 10 custom fields', function() {
        it('Add button is disabled', function() {
          ...
          cy.get('.orangehrm-custom-field-title > .oxd-text--p').should('include.text', 'All custom fields are in use');
        });
        after(() => {
            var i = 0;
            for (i = 0; i < 10 ; i++) { 
