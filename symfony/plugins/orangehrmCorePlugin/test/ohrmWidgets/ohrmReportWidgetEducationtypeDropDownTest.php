...
class ohrmReportWidgetEducationtypeDropDownTest extends PHPUnit\Framework\TestCase
{
    ...
    {
        $this->ohrmReportWidgetEducationtypeDropDown = $this->getMockBuilder(ohrmReportWidgetEducationtypeDropDown::class)
    ...
    {
        $returnValue = $this->ohrmReportWidgetEducationtypeDropDown->generateWhereClausePart('fieldName', '1');
        $this->assertEquals("fieldName = '1'",$returnValue);
    }

    public function testGenerateWhereClausePartWithSql()
    {
        $returnValue = $this->ohrmReportWidgetEducationtypeDropDown->generateWhereClausePart('fieldName', '1;DELETE FROM `hs_hr_employee` WHERE `hs_hr_employee`.`emp_number` = 1;');
