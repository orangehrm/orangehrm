...
class HelpConfigServiceTest extends PHPUnit\Framework\TestCase
{
    ...
    public function testGetHelpProcessorClass()
    {
        $mockHelpConfigService = $this->getMockBuilder('HelpConfigService')
            ...
            ->getMock();
        $mockHelpConfigService->expects($this->once())
            ->method('_getConfigValue')
            ->with('help.processorClass')
            ->will($this->returnValue('ZendeskHelpProcessor'));

        $expected = 'ZendeskHelpProcessor';
        $config = $mockHelpConfigService->getHelpProcessorClass();
    ...
    public function testGetBaseHelpUrl()
    {
        ...
        $expected = $baseUrl;
        $config = $mockHelpConfigService->getBaseHelpUrl();
        $this->assertEquals($expected, $config);
    }
