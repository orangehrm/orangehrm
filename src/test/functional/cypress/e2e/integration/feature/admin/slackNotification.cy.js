...
describe('Admin - Slack Notification Configuration', function () {
  ...
  describe('create slack registration', function () {
    // TODO(slack-cypress): the test below was written against an earlier Vue
    ...
    it.skip('admin can create a birthday notification for a Slack channel', function () {
      cy.loginTo(this.user, '/admin/slackNotificationConfiguration');
      cy.wait([
        '@getConfig',
        '@getRegistrations',
  ...
  describe('delete slack registration', function () {
    // TODO(slack-cypress): this test references `cy.task('db:restore', {name:
    // 'slackRegistrationCreated'})` but that savepoint is never created — the
    // create test above would need to call `cy.task('db:snapshot', {name:
    // 'slackRegistrationCreated'})` first, per the OHRM Cypress pattern.
    ...
    // covered by SlackRegistrationAPITest (PHPUnit) so the contract is pinned.
  ...
  describe('access control', function () {
    // TODO(slack-cypress): asserting `.orangehrm-main-title` does NOT contain
    ...
    // .should('not.contain', '/admin/slackNotificationConfiguration')`) but
    ...
    // eslint-disable-next-line jest/no-disabled-tests -- see TODO above
    it.skip('ESS user is redirected away from the slack page', function () {
