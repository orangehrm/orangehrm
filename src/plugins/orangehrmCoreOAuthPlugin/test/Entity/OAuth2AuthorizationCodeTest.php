...
class OAuth2AuthorizationCodeTest extends EntityTestCase
{
    ...
    public function testOAuth2AccessTokenEntityTest(): void
    {
        ...
        $authorizationCode->setAuthorizationCode('Authorization Code');
        $authorizationCode->setExpiryDateTime($date);
        $authorizationCode->setUserIdentifier($user->getId());
        $authorizationCode->setClient($client);
        $authorizationCode->setRedirectUri('redirect.com');
        $authorizationCode->setScopes([$scope]);
        $this->persist($authorizationCode);

        /** @var OAuth2AuthorizationCode $authorizationCode */
        $authorizationCode = $this->getRepository(OAuth2AuthorizationCode::class)->find(1);
        $this->assertEquals('Authorization Code', $authorizationCode->getAuthorizationCode());
        $this->assertEquals($date, $authorizationCode->getExpiryDateTime());
