...
class BuzzLikeOnCommentTest extends EntityTestCase
{
    ...
    {
        $likeOnComment =  new BuzzLikeOnComment();
        $likeOnComment->setEmployee($this->getReference(Employee::class, 1));
//        need to add share
        $likeOnComment->setLikedAt(new \DateTime('2022-11-01 09:20'));

        $this->assertEquals('2022-11-01', $likeOnComment->getLikedAt()->format('Y-m-d'));
    }
}
