 ...
 * @group Dao
 */
class SlackRegistrationDaoTest extends TestCase
{
    private SlackRegistrationDao $dao;
    ...
    {
        // SlackBaseFixture only seeds subunits — no registrations yet.
    ...
    {
        // The scheduler only iterates active rows — pinning that filter is critical.
    ...
    public function testGetRegistrationByIdReturnsEntity(): void
    {
        ...
        $this->assertInstanceOf(SlackRegistration::class, $found);
    ...
    public function testSaveRegistrationAssignsIdAndRoundTrips(): void
    {
        ...
        $this->assertInstanceOf(SlackRegistration::class, $reloaded);
        ...
        $this->assertSame(SlackRegistration::PROVIDER_SLACK, $reloaded->getProvider());
        $this->assertTrue($reloaded->isActive());
    }

    public function testSaveRegistrationPersistsSubunitsJoinTable(): void
    {
        // Multi-subunit support is the whole reason the join table exists — pin it.
    ...
    {
        // Mirrors the production update flow (SlackRegistrationService::syncSubunits):
        // re-fetch the registration, clear, re-add a different subunit set, persist.
    ...
    private function makeRegistration(string $eventType, string $channelLabel, bool $active = true): SlackRegistration
    {
    ...
    private function saveBasicRegistration(string $eventType, string $channelLabel, bool $active = true): SlackRegistration
    {
