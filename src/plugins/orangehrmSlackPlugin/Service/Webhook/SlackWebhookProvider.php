...
/**
 * Slack-specific webhook implementation. Knows the `hooks.slack.com` URL shape
 * and Slack's plain-text `ok` success-response convention. Was named
 * `SlackWebhookClient` before the multi-provider seam was introduced.
 */
class SlackWebhookProvider implements WebhookProviderInterface
{
    ...
    {
        return SlackRegistration::PROVIDER_SLACK;
    }
    ...
    /**
     * Keep workspace + channel ids (low-sensitivity, useful for the admin to
     * recognise the row); strip the trailing webhook secret.
     *
     *   https://hooks.slack.com/services/T01ABC/B02DEF/abc123secret
     *                                            ↓
     *   https://hooks.slack.com/services/T01ABC/B02DEF/…
     *
     * The FE mirror at SlackNotificationConfiguration.vue:286-289 produces
     * the same shape — duplicate-detection compares the two byte-for-byte.
     */
    public function maskUrl(string $url): string
    {
        ...
        // URL doesn't match the canonical Slack shape — fall back to the safe
        // "drop the last path segment" rule rather than echoing the raw URL.
        return self::genericMask($url);
    }

    /**
     * Last-resort masker for inputs that don't match a known provider's shape.
     * Public so the registry dispatcher can use the same fallback when no
     * provider id is available (e.g. legacy rows with provider=NULL).
     */
    ...
    public function send(string $webhookUrl, string $text): WebhookDeliveryResult
    {
        try {
            ...
            // Some Guzzle handler configurations (and curl verbose mode) embed
            // the full request URL in the exception message — scrub it before
            // the message flows into slack_log.error_message.
        ...
        // Defensive: response bodies aren't expected to echo the URL back, but
        // we scrub anyway — a future server change couldn't leak silently.
     ...
     * masked form. Shared inside the provider so the masking rule used in
     * logs / failure messages stays in lockstep with {@see maskUrl()}.
     */
