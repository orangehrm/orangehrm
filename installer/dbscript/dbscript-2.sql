...
INSERT INTO ohrm_user_role_data_group (`can_read`, `can_create`, `can_update`, `can_delete`, `self`, `data_group_id`, `user_role_id`) VALUES (1, 1, 1, 1, 0, @apiv2_performance_performance-trackerId_data_group_id, @admin_role_id);

INSERT INTO ohrm_data_group (`name`, `description`, `can_read`, `can_create`, `can_update`, `can_delete`) VALUES
    ('reviewer_list', 'Performance - reviewers list', 1, 0, 0, 0);

SET @performance_module_id := (SELECT `id` FROM ohrm_module WHERE name = 'performance' LIMIT 1);

 SET @reviewer_list_data_group_id := (SELECT `id` FROM ohrm_data_group WHERE name = 'reviewer_list' LIMIT 1);

INSERT INTO ohrm_api_permission (`api_name`, `module_id`, `data_group_id`) VALUES ('OrangeHRM\\Performance\\Api\\PerformanceTrackerReviewerAPI', @performance_module_id, @reviewer_list_data_group_id);

SET @reviewer_list_data_group_id := (SELECT `id` FROM ohrm_data_group WHERE name = 'reviewer_list' LIMIT 1);

SET @admin_role_id := (SELECT `id` FROM ohrm_user_role WHERE `name` = 'Admin' LIMIT 1);

INSERT INTO ohrm_user_role_data_group (`can_read`, `can_create`, `can_update`, `can_delete`, `self`, `data_group_id`, `user_role_id`) VALUES (1, 0, 0, 0, 0, @reviewer_list_data_group_id, @admin_role_id);
